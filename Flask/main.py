...
def recommend():
    try:
        ...
        # Define the CSV file path (ensure the file exists in production)
        csv_file_path = os.path.join(os.path.dirname(__file__), '../GeneticAlgorithm/rfg_updated.csv')

        # Instantiate the MealRecommender and get recommendations
        recommender = MealRecommender(
            csv_file_path, target, meal_count=meal_count, population_size=20, 
            generation_limit=500, mutation_rate=0.4, k=1, dietery=dietery
        )
